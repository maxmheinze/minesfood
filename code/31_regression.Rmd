...
* the croplands EVI (based on Africover mask) by **`r round(coef(mod2_order_meanEVI[[2]])["as.factor(order)::0:downstream"] / evi_cropland_africover_avg * 100, 2)`%**, p-value = *`r round(pvalue(mod2_order_meanEVI[[2]])["as.factor(order)::0:downstream"], 3)`* (`r round(coef(mod4_order_meanEVI[[2]])["as.factor(order)::0:downstream"] / evi_cropland_africover_avg * 100, 2)`%, p-value = *`r round(pvalue(mod4_order_meanEVI[[2]])["as.factor(order)::0:downstream"], 3)`*)
...
```{r plot_reg_placebo, warning=FALSE, message=FALSE}
...
coefplot(mod2_order_plot, ci_level = 0.9, keep = "downstream",
