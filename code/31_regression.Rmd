...
type <- "centroid" # c("", "ipis", "centroid") cent with restr_order = 10 to get pre-revision sample
order <- 10
...
# covariates 
covs <- c("elevation", "slope", "soilgrid_grouped", "tmp_max", "precipitation", "accessibility_to_cities_2015", "pop_2015") 
# 
...
# load data
df_reg <- readRDS(p("processed/df_reg", 
                         "-order-", order,
...
* up/downstream basins up to **`r max(df_reg_restr$dist_n)`** basins "away"
...
mine_number_restr <- df_reg_restr |> filter(year == restr_year[1], dist_n != 0) |> 
  group_by(mine_basin, downstream, dist_n) |> count() |> 
  group_by(mine_basin, downstream) |> count() |> 
  filter(n >= 1) |> 
  group_by(mine_basin) |> count() |> 
