...
mod_order_hetero_biome = feols(c(max_EVI, max_c_EVI_af, max_c_EVI_ESA, cropland_yield_kg_ha) ~
                           i(as.factor(order), downstream)  +
                           elevation + slope + soilgrid_grouped +
                           tmp_max + precipitation +
                           accessibility_to_cities_2015 + pop_2015 | 
...
mod_dist_hetero_biome = feols(c(max_EVI, max_c_EVI_af, max_c_EVI_ESA, cropland_yield_kg_ha) ~
                           (distance + I(distance^2)) * downstream  +
                           elevation + slope + soilgrid_grouped +
                           tmp_max + precipitation  +
                           accessibility_to_cities_2015 + pop_2015 |
...
mod2_order_plot = feols(c(tmp_mean, tmp_max, tmp_min, frs_days) ~ 
                          i(factor(order), downstream) |
                          year +  as.factor(mine_basin),
                        data = df_reg_restr,
                        cluster = "HYBAS_ID")
