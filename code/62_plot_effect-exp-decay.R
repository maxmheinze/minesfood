...
# newdat <- data.frame(dist_km = seq(-25, 200, by = 1), downstream = 0) |>
#   mutate(downstream = ifelse(dist_km < 0, downstream, 1))
# predictions_evi <- lapply(mods$EVI, function(x) predict(x, newdata = newdat)) |>
#   bind_cols() |> mutate(dist_km = -25:200) |> tidyr::pivot_longer(cols = contains("exp"))
# ggplot(predictions_evi |> filter(name == min_evi_bic_name), aes(x = dist_km, y = value)) +
#   geom_point()
...
#   bind_cols() |> mutate(dist_km = -25:200) |> tidyr::pivot_longer(cols = contains("exp"))
# ggplot(predictions_evi_c |> filter(name == min_evi_c_bic_name), aes(x = dist_km, y = value)) +
#   geom_point()
